...
class TheCompletionListType {
    ...
    @Test
    @DisplayName("supports a completion item array")
    fun supports_a_completion_item_array() {
        ...
        val list = CompletionList.deserialize(json)
        assertEquals(null, list.isIncomplete)
        assertEquals(1, list.items.size)
    ...
    fun throws_an_error_if_the_kind_type_is_not_supported() {
        val e1 = assertFails { CompletionList.deserialize(JsonNull) }
        assertEquals(UnsupportedKindTypeException::class, e1::class)
        assertEquals("Unsupported kind type 'null'", e1.message)

        val e2 = assertFails { CompletionList.deserialize(JsonPrimitive("test")) }
        assertEquals(UnsupportedKindTypeException::class, e2::class)
        assertEquals("Unsupported kind type 'string'", e2.message)

        val e3 = assertFails { CompletionList.deserialize(JsonPrimitive(true)) }
        assertEquals(UnsupportedKindTypeException::class, e3::class)
        assertEquals("Unsupported kind type 'boolean'", e3.message)

        val e4 = assertFails { CompletionList.deserialize(JsonPrimitive(1)) }
        assertEquals(UnsupportedKindTypeException::class, e4::class)
        assertEquals("Unsupported kind type 'integer'", e4.message)

        val e5 = assertFails { CompletionList.deserialize(JsonPrimitive(1.2)) }
