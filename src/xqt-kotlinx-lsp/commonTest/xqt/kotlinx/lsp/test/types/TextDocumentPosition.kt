...
@DisplayName("The TextDocumentPosition type")
class TheTextDocumentPositionType {
    @Test
    ...
    fun supports_the_non_optional_properties() {
        ...
        val tdpp = TextDocumentPosition.deserialize(json)
        assertEquals("file:///home/lorem/ipsum.py", tdpp.uri)
        assertEquals(2u, tdpp.position.line)
        assertEquals(6u, tdpp.position.character)

        assertEquals(json, TextDocumentPosition.serializeToJson(tdpp))
    }

    @Test
    @DisplayName("throws an error if the kind type is not supported")
    fun throws_an_error_if_the_kind_type_is_not_supported() {
        val e1 = assertFails { TextDocumentPosition.deserialize(JsonArray(arrayListOf())) }
        assertEquals(UnsupportedKindTypeException::class, e1::class)
        assertEquals("Unsupported kind type 'array'", e1.message)

        val e2 = assertFails { TextDocumentPosition.deserialize(JsonNull) }
        assertEquals(UnsupportedKindTypeException::class, e2::class)
        assertEquals("Unsupported kind type 'null'", e2.message)

        val e3 = assertFails { TextDocumentPosition.deserialize(JsonPrimitive("test")) }
        assertEquals(UnsupportedKindTypeException::class, e3::class)
        assertEquals("Unsupported kind type 'string'", e3.message)

        val e4 = assertFails { TextDocumentPosition.deserialize(JsonPrimitive(true)) }
        assertEquals(UnsupportedKindTypeException::class, e4::class)
        assertEquals("Unsupported kind type 'boolean'", e4.message)

        val e5 = assertFails { TextDocumentPosition.deserialize(JsonPrimitive(1)) }
        assertEquals(UnsupportedKindTypeException::class, e5::class)
        assertEquals("Unsupported kind type 'integer'", e5.message)

        val e6 = assertFails { TextDocumentPosition.deserialize(JsonPrimitive(1.2)) }
